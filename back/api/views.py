...
def google_login_callback(request):
    ...
    if not social_account:
        return redirect(os.getenv("FRONTEND_URL", "http://localhost:5173") + "/login/callback/?error=NoSocialAccount")
    try:
        token = SocialToken.objects.get(account=social_account)
        refresh = RefreshToken.for_user(user)
        access_token = str(refresh.access_token)
        user.profile_picture = social_account.extra_data.get('picture', user.profile_picture)
        user.name = social_account.extra_data.get('name', user.name)
        user.save()
        return redirect(os.getenv("FRONTEND_URL", "http://localhost:5173") + f"/login/callback/?access_token={access_token}&refresh_token={str(refresh)}")
    except SocialToken.DoesNotExist:
        return redirect(os.getenv("FRONTEND_URL", "http://localhost:5173") + "/login/callback/?error=NoGoogleToken")
    except Exception as e:
        logger.error(f"Error in google_login_callback: {str(e)}")
        return redirect(os.getenv("FRONTEND_URL", "http://localhost:5173") + "/login/callback/?error=ServerError")
...
def google_login(request):
    ...
    try:
        ...
        if not google_id_token_from_frontend:
            ...
            return JsonResponse({"detail": "No token provided in 'access_token' field"}, status=400)
        ...
        if idinfo['aud'] in client_ids:
            aud_is_valid = True
        if not aud_is_valid:
            logger.error(f"Token audience verification failed. aud: {idinfo.get('aud')}, expected one of: {client_ids}")
            raise ValueError(f"Could not verify audience. Received: {idinfo.get('aud')}")
        email = idinfo["email"]
        name = idinfo.get("name", "")
        profile_picture = idinfo.get("picture", None)
        user, created = User.objects.get_or_create(
            email=email,
            defaults={'name': name, 'profile_picture': profile_picture}
        )
        if not created:
            user.name = name
        ...
        return JsonResponse({"detail": "Server error occurred during Google login"}, status=500)
...
def serve_profile_picture(request, user_id):
    ...
    try:
        ...
        logger.error(f"Could not fetch profile picture for user {user_id} from {user.profile_picture}: {e}")
        return HttpResponse(status=status.HTTP_503_SERVICE_UNAVAILABLE, content="Could not fetch profile picture.")
    except Exception as e:
...
def create_profile(request):
    ...
    if profile_exists:
        return Response({"detail": "Profile already exists. Use PUT/PATCH on 'profile/me/' to update."}, status=status.HTTP_400_BAD_REQUEST)
...
class MyUserProfileView(generics.RetrieveUpdateAPIView):
    ...
    def perform_update(self, serializer):
        serializer.save(user=self.request.user, form_completed=True)
...
def check_profile(request):
    try:
        ...
        return Response({"form_completed": profile.form_completed, "profile_exists": True})
...
class FollowUserView(drf_views.APIView):
    ...
    def post(self, request, user_id):
        ...
        return Response({"detail": f"You are now following {user_to_follow.name}."}, status=status.HTTP_200_OK)
...
class UnfollowUserView(drf_views.APIView):
    ...
    def post(self, request, user_id):
        ...
        return Response({"detail": f"You have unfollowed {user_to_unfollow.name}."}, status=status.HTTP_200_OK)
